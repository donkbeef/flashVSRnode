...
If you are already inside the cloned node folder, install from the local file instead:

```bash
cd ComfyUI/custom_nodes/ComfyUI-FlashVSR_Stable
python -m pip install -r requirements.txt
```

For ComfyUI portable on Windows, use ComfyUI's embedded Python so packages are installed into the same environment that runs ComfyUI:

```powershell
..\..\..\python_embeded\python.exe -m pip install -r requirements.txt
```

From any Windows folder, use absolute paths:

```powershell
E:\ComfyUI_windows_portable\python_embeded\python.exe -m pip install -r E:\ComfyUI_windows_portable\ComfyUI\custom_nodes\ComfyUI-FlashVSR_Stable\requirements.txt
```
...
> 📢 **Linux only**: If `flash-attn` needs a source build in your environment, install it separately after the main requirements:
> ```bash
> python -m pip install flash-attn --no-build-isolation
> ```

> 📢 **Windows note**: `sageattention` is optional on Windows. If an installed `sageattention` wheel is not compatible with your Torch/CUDA build, FlashVSR will now skip it and fall back to PyTorch attention instead of failing during ComfyUI startup.
