...
try:
    ...
    print(f"[FlashVSR] flash_attn_interface unavailable, falling back to other attention kernels: {e}")
...
try:
    ...
    print(f"[FlashVSR] flash_attn unavailable, falling back to other attention kernels: {e}")
    flash_attn = None
...
try:
    ...
    print(f"[FlashVSR] sageattention unavailable, falling back to PyTorch attention: {e}")
    sageattn = None
...
try:
    ...
    print(f"[FlashVSR] block_sparse_attn unavailable, falling back to sparse_sage/PyTorch attention: {e}")
...
try:
    ...
    print(f"[FlashVSR] local sparse_sage attention unavailable, falling back to PyTorch attention: {e}")
        ...
        if USE_BLOCK_ATTN and BLOCK_ATTN_AVAILABLE:
            ...
            try:
                if not SPARSE_SAGE_AVAILABLE:
                    raise RuntimeError("local sparse_sage attention is not available")
                x = sparse_sageattn(
                    q, k, v,
                ...
                print(f"[FlashVSR] sparse_sage attention failed, falling back to PyTorch attention: {e}")
